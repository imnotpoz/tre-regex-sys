...
mod tests {
    ...
    fn regcomp() {
        unsafe {
            ...
            if tre_regcomp(preg.as_mut_ptr(), b"Hello(, [[:alpha:]]+)?!\0".as_ptr() as * const _, (REG_EXTENDED | REG_ICASE) as i32) != 0 {
                panic!("tre_regcomp");
            }
            ...
            if tre_regexec(&preg, b"Hello!".as_ptr() as * const _, nmatch, pmatch.as_mut_ptr(), 0) != 0 {
                panic!("tre_regexec");
            }
            ...
            if tre_regexec(&preg, b"Hello, world!".as_ptr() as * const _, nmatch, pmatch.as_mut_ptr(), 0) != 0 {
                panic!("tre_regexec");
            }
